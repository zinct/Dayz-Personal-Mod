class ActionRepairCarPartWithEpoxyCB : ActionRepairCarPartCB
{
...
class ActionRepairCarPartWithEpoxy : ActionRepairCarPart 
{
	void ActionRepairCarPartWithEpoxy()
	{
		m_CallbackClass 	= ActionRepairCarPartWithEpoxyCB;
	...
	override bool ActionCondition(PlayerBase player, ActionTarget target, ItemBase item)
	{
		...
		if (carDoor)
		{
			...
			return zoneHP > GameConstants.STATE_PRISTINE && zoneHP < GameConstants.STATE_RUINED);
		}
	...
	override void OnFinishProgressServer(ActionData action_data)
	{
		...
		if (tgObject)
		{
			...
			if (carDoor)
			{
				int newDmgLevel = Math.Clamp(carDoor.GetHealthLevel(damageZone) - 1, GameConstants.STATE_PRISTINE, GameConstants.STATE_RUINED);
